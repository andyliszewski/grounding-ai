...
### Added
- `ChunkConfig.min_chunk_size` and `--min-chunk-size` (default: auto, 200 capped at `chunk_size/4`; `0` disables). Chunks shorter than the threshold are folded into the chunk that follows them, so an orphaned caption or title stays with the block it names.
...
### Fixed
- Orphaned single-line chunks stranded table captions and section titles away from their content. The splitter emits one chunk per separator-delimited block that will not fit the current window, so a short block between two full ones was emitted alone: the caption embedded as a bare title carrying none of the data it named, while the data embedded carrying nothing that said what it was. A retrieval query matching the caption could never reach the numbers. On a 47 MB engineering handbook this affected 2,218 of 18,196 chunks (12%); after the fix, none. Note that merging an orphan into an already-full neighbour necessarily exceeds `chunk_size`; growth is bounded at `chunk_size + min_chunk_size`, and the existing size-contract test was updated to that bound.
